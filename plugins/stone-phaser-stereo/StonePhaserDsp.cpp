  ...
  public:
	...
	void fillmydspSIG0(int count, float* output) {
		for (int i = 0; (i < count); i = (i + 1)) {
			iRec11[0] = (iRec11[1] + 1);
			float fTemp0 = (0.0078125f * float((iRec11[0] + -1)));
...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("stone_phaser.dsp/version", "1.2.1");
	...
	virtual void instanceConstants(int samplingFreq) {
		...
		fConst1 = std::exp((0.0f - (207.345108f / fConst0)));
		fConst2 = (1.0f / fConst0);
		fConst3 = (0.5f * (fConst1 + 1.0f));
		fConst4 = (0.0f - fConst3);
		fConst5 = (2764.60156f / fConst0);
	...
	virtual void instanceClear() {
		...
		for (int l14 = 0; (l14 < 4); l14 = (l14 + 1)) {
			fRec19_perm[l14] = 0.0f;
			
		}
		for (int l15 = 0; (l15 < 4); l15 = (l15 + 1)) {
			fRec20_perm[l15] = 0.0f;
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("Color", &fHslider3, 1.0f, 0.0f, 1.0f, 1.0f);
		ui_interface->declare(&fHslider2, "2", "");
		ui_interface->declare(&fHslider2, "scale", "log");
		ui_interface->declare(&fHslider2, "unit", "Hz");
		ui_interface->addHorizontalSlider("LFO frequency", &fHslider2, 0.200000003f, 0.00999999978f, 5.0f, 0.00999999978f);
		...
		ui_interface->addHorizontalSlider("Feedback bass cut", &fHslider1, 500.0f, 10.0f, 5000.0f, 1.0f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow4 = (0.0157079641f * float(fHslider4));
		float fSlow5 = (0.00100000005f * std::sin(fSlow4));
		float fRec12_tmp[36];
		float* fRec12 = &fRec12_tmp[4];
		float fSlow6 = (0.00100000005f * std::cos(fSlow4));
		float fRec13_tmp[36];
		...
		float fSlow8 = (2.77777781e-06f * float(fHslider5));
		float fRec21_tmp[36];
		...
		for (vindex = 0; (vindex <= (count - 32)); vindex = (vindex + 32)) {
			...
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fRec10[i] = (fSlow2 + (0.999000013f * fRec10[(i - 1)]));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec1[i] = (fRec9[(i - 1)] + (fConst2 * fRec10[(i - 1)]));
				fRec9[i] = (fZec1[i] - std::floor(fZec1[i]));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fRec21[i] = (fSlow8 + (0.999000013f * fRec21[(i - 1)]));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec10[i] = (fRec9[i] + fRec21[i]);
				
			}
			/* Vectorizable loop 4 */
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec5[i] = (128.0f * fRec9[i]);
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fRec8[i] = (fSlow1 + (0.999000013f * fRec8[(i - 1)]));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec0[i] = std::exp((fConst2 * (0.0f - (6.28318548f * fRec8[i]))));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec3[i] = (fZec0[i] + 1.0f);
				
			}
			/* Vectorizable loop 14 */
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec8[i] = ((ftbl0mydspSIG0[iZec6[i]] * (fZec7[i] + (1.0f - fZec5[i]))) + ((fZec5[i] - fZec7[i]) * ftbl0mydspSIG0[((iZec6[i] + 1) % 128)]));
				
			}
			/* Vectorizable loop 15 */
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec14[i] = ((ftbl0mydspSIG0[iZec12[i]] * (fZec13[i] + (1.0f - fZec11[i]))) + ((fZec11[i] - fZec13[i]) * ftbl0mydspSIG0[((iZec12[i] + 1) % 128)]));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fRec5[i] = ((fConst1 * fRec5[(i - 1)]) + float(input0[i]));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec2[i] = (iSlow3?fRec6[i]:(0.100000001f * fRec6[i]));
				
			}
			/* Vectorizable loop 18 */
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec4[i] = (0.0f - (0.5f * fZec3[i]));
				
			}
			/* Vectorizable loop 19 */
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec9[i] = ((fConst5 * std::pow(2.0f, (0.0833333358f * ((iSlow3?((57.3763161f * fZec8[i]) + 39.4868202f):((51.8631363f * fZec8[i]) + 62.3695068f)) + -69.0f)))) + -1.0f);
				
			}
			/* Recursive loop 20 */
			/* Pre code */
			for (int j26 = 0; (j26 < 4); j26 = (j26 + 1)) {
				fRec19_tmp[j26] = fRec19_perm[j26];
				
			}
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fRec19[i] = ((fConst1 * fRec19[(i - 1)]) + float(input1[i]));
				
			}
			/* Post code */
			for (int j27 = 0; (j27 < 4); j27 = (j27 + 1)) {
				fRec19_perm[j27] = fRec19_tmp[(vsize + j27)];
				
			}
			/* Vectorizable loop 21 */
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec15[i] = ((fConst5 * std::pow(2.0f, (0.0833333358f * ((iSlow3?((57.3763161f * fZec14[i]) + 39.4868202f):((51.8631363f * fZec14[i]) + 62.3695068f)) + -69.0f)))) + -1.0f);
			...
			for (int j12 = 0; (j12 < 4); j12 = (j12 + 1)) {
				fRec4_tmp[j12] = fRec4_perm[j12];
				
			}
			for (int j14 = 0; (j14 < 4); j14 = (j14 + 1)) {
				fRec3_tmp[j14] = fRec3_perm[j14];
				
			}
			for (int j16 = 0; (j16 < 4); j16 = (j16 + 1)) {
				fRec2_tmp[j16] = fRec2_perm[j16];
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fRec7[i] = (fRec0[(i - 1)] + (fRec7[(i - 1)] * fZec0[i]));
				fRec4[i] = (((fConst4 * fRec5[(i - 1)]) + ((fZec2[i] * ((0.5f * (fRec7[i] * fZec3[i])) + (fRec7[(i - 1)] * fZec4[i]))) + (fConst3 * fRec5[i]))) - (fRec4[(i - 1)] * fZec9[i]));
				fRec3[i] = (fRec4[(i - 1)] + (fZec9[i] * (fRec4[i] - fRec3[(i - 1)])));
				fRec2[i] = (fRec3[(i - 1)] + (fZec9[i] * (fRec3[i] - fRec2[(i - 1)])));
				fRec1[i] = (fRec2[(i - 1)] + (fZec9[i] * (fRec2[i] - fRec1[(i - 1)])));
				fRec0[i] = (fRec1[(i - 1)] + (fRec1[i] * fZec9[i]));
			...
			for (int j13 = 0; (j13 < 4); j13 = (j13 + 1)) {
				fRec4_perm[j13] = fRec4_tmp[(vsize + j13)];
				
			}
			for (int j15 = 0; (j15 < 4); j15 = (j15 + 1)) {
				fRec3_perm[j15] = fRec3_tmp[(vsize + j15)];
				
			}
			for (int j17 = 0; (j17 < 4); j17 = (j17 + 1)) {
				fRec2_perm[j17] = fRec2_tmp[(vsize + j17)];
			...
			for (int j22 = 0; (j22 < 4); j22 = (j22 + 1)) {
				fRec12_tmp[j22] = fRec12_perm[j22];
			...
			for (int j23 = 0; (j23 < 4); j23 = (j23 + 1)) {
				fRec12_perm[j23] = fRec12_tmp[(vsize + j23)];
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fRec20[i] = (fRec14[(i - 1)] + (fZec0[i] * fRec20[(i - 1)]));
				fRec18[i] = (((fConst4 * fRec19[(i - 1)]) + ((fZec2[i] * ((0.5f * (fRec20[i] * fZec3[i])) + (fZec4[i] * fRec20[(i - 1)]))) + (fConst3 * fRec19[i]))) - (fRec18[(i - 1)] * fZec15[i]));
				fRec17[i] = (fRec18[(i - 1)] + (fZec15[i] * (fRec18[i] - fRec17[(i - 1)])));
				fRec16[i] = (fRec17[(i - 1)] + (fZec15[i] * (fRec17[i] - fRec16[(i - 1)])));
				fRec15[i] = (fRec16[(i - 1)] + (fZec15[i] * (fRec16[i] - fRec15[(i - 1)])));
				fRec14[i] = (fRec15[(i - 1)] + (fRec15[i] * fZec15[i]));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				output0[i] = FAUSTFLOAT((iSlow7?float(input0[i]):((fRec0[i] * fRec12[i]) + (fRec13[i] * float(input0[i])))));
				
			}
			/* Vectorizable loop 27 */
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				output1[i] = FAUSTFLOAT((iSlow7?float(input1[i]):((fRec13[i] * float(input1[i])) + (fRec12[i] * fRec14[i]))));
	...
	FAUST_ADDHORIZONTALSLIDER("Color", fHslider3, 1.0f, 0.0f, 1.0f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("LFO frequency", fHslider2, 0.20000000000000001f, 0.01f, 5.0f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("Feedback depth", fHslider0, 75.0f, 0.0f, 99.0f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("Feedback bass cut", fHslider1, 500.0f, 10.0f, 5000.0f, 1.0f);
...
namespace mydsp_meta {
	...
	FMSTATIC const char *const active_symbol[] = {u8"Bypass", u8"Color", u8"LFO_frequency", u8"Feedback_depth", u8"Feedback_bass_cut", u8"Dry_wet_mix", u8"Stereo_phase"};
	FMSTATIC const std::size_t active_offsets[] = {(size_t)&((FAUSTCLASS *)0)->fCheckbox0, (size_t)&((FAUSTCLASS *)0)->fHslider3, (size_t)&((FAUSTCLASS *)0)->fHslider2, (size_t)&((FAUSTCLASS *)0)->fHslider0, (size_t)&((FAUSTCLASS *)0)->fHslider1, (size_t)&((FAUSTCLASS *)0)->fHslider4, (size_t)&((FAUSTCLASS *)0)->fHslider5};
	...
	FMSTATIC const metadata_t *const active_metadata[] = {(metadata_t[]){}, (metadata_t[]){}, (metadata_t[]){{u8"scale", u8"log"}, {u8"unit", u8"Hz"}}, (metadata_t[]){{u8"unit", u8"%"}}, (metadata_t[]){{u8"scale", u8"log"}, {u8"unit", u8"Hz"}}, (metadata_t[]){{u8"unit", u8"%"}}, (metadata_t[]){{u8"unit", u8"deg"}}};
	FMSTATIC constexpr std::size_t active_metadata_size[] = {0, 0, 2, 1, 2, 1, 1};
	...
	FMSTATIC inline void set_Color(FAUSTCLASS &x, FAUSTFLOAT v) { x.fHslider3 = v; }
	FMSTATIC inline void set_LFO_frequency(FAUSTCLASS &x, FAUSTFLOAT v) { x.fHslider2 = v; }
	FMSTATIC inline void set_Feedback_depth(FAUSTCLASS &x, FAUSTFLOAT v) { x.fHslider0 = v; }
	FMSTATIC inline void set_Feedback_bass_cut(FAUSTCLASS &x, FAUSTFLOAT v) { x.fHslider1 = v; }
	...
	FMSTATIC inline FAUSTFLOAT get_Color(const FAUSTCLASS &x) { return x.fHslider3; }
	FMSTATIC inline FAUSTFLOAT get_LFO_frequency(const FAUSTCLASS &x) { return x.fHslider2; }
	FMSTATIC inline FAUSTFLOAT get_Feedback_depth(const FAUSTCLASS &x) { return x.fHslider0; }
	FMSTATIC inline FAUSTFLOAT get_Feedback_bass_cut(const FAUSTCLASS &x) { return x.fHslider1; }
