 ...
 public:
	...
	FAUSTPP_VIRTUAL void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (vindex = 0; (vindex <= (count - 32)); vindex = (vindex + 32)) {
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec6[i] = ((fConst8 * std::pow(2.0f, (0.0833333358f * ((fRec12[i] + ((fRec13[i] - fRec12[i]) * ((ftbl0mydspSIG0[iZec4[i]] * (fZec5[i] + (1.0f - fZec3[i]))) + ((fZec3[i] - fZec5[i]) * ftbl0mydspSIG0[((iZec4[i] + 1) % 128)])))) + -69.0f)))) + -1.0f);
			}
			/* Recursive loop 15 */
			/* Pre code */
			...
			for (int i = 0; (i < vsize); i = (i + 1)) {
				...
				fRec6[i] = (((fConst7 * fRec7[(i - 1)]) + ((fRec8[i] * ((0.5f * (fRec10[i] * fZec2[i])) + (fRec10[(i - 1)] * (0.0f - (0.5f * fZec2[i]))))) + (fConst6 * fRec7[i]))) - (fRec6[(i - 1)] * fZec6[i]));
				fRec5[i] = (fRec6[(i - 1)] + (fZec6[i] * (fRec6[i] - fRec5[(i - 1)])));
				fRec4[i] = (fRec5[(i - 1)] + (fZec6[i] * (fRec5[i] - fRec4[(i - 1)])));
				fRec3[i] = (fRec4[(i - 1)] + (fZec6[i] * (fRec4[i] - fRec3[(i - 1)])));
				fRec2[i] = (fRec3[(i - 1)] + (fRec3[i] * fZec6[i]));
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec7[i] = (1.0f - fRec0[i]);
			}
			/* Vectorizable loop 19 */
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fHbargraph0 = FAUSTFLOAT(fZec7[i]);
				output0[i] = FAUSTFLOAT(((fZec7[i] * ((float(input0[i]) * fRec1[i]) + (fRec2[i] * fRec17[i]))) + (float(input0[i]) * fRec0[i])));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
