 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (vindex = 0; (vindex <= (count - 32)); vindex = (vindex + 32)) {
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec6[i] = ((ftbl0mydspSIG0[((iZec4[i] + 1) % 128)] * (fZec3[i] - fZec5[i])) + (ftbl0mydspSIG0[iZec4[i]] * (fZec5[i] + (1.0f - fZec3[i]))));
			...
			/* Compute code */
			for (int i = 0; (i < vsize); i = (i + 1)) {
				fZec7[i] = ((fConst5 * std::pow(2.0f, (0.0833333358f * ((iSlow3?((57.3763161f * fZec6[i]) + 39.4868202f):((51.8631363f * fZec6[i]) + 62.3695068f)) + -69.0f)))) + -1.0f);
			...
			for (int i = 0; (i < vsize); i = (i + 1)) {
				...
				fRec4[i] = (((fConst4 * fRec5[(i - 1)]) + (((iSlow3?fRec6[i]:(0.100000001f * fRec6[i])) * ((0.5f * (fRec7[i] * fZec2[i])) + (fRec7[(i - 1)] * (0.0f - (0.5f * fZec2[i]))))) + (fConst3 * fRec5[i]))) - (fRec4[(i - 1)] * fZec7[i]));
				fRec3[i] = (fRec4[(i - 1)] + (fZec7[i] * (fRec4[i] - fRec3[(i - 1)])));
				fRec2[i] = (fRec3[(i - 1)] + (fZec7[i] * (fRec3[i] - fRec2[(i - 1)])));
				fRec1[i] = (fRec2[(i - 1)] + (fZec7[i] * (fRec2[i] - fRec1[(i - 1)])));
				fRec0[i] = (fRec1[(i - 1)] + (fRec1[i] * fZec7[i]));
...
namespace mydsp_meta {
	...
	FMSTATIC constexpr char version[] = u8"1.2.1";
	...
	FMSTATIC const metadata_t metadata[] = {{u8"author", u8"Jean Pierre Cimalando"}, {u8"basics.lib/name", u8"Faust Basic Element Library"}, {u8"basics.lib/version", u8"0.0"}, {u8"filename", u8"stone_phaser"}, {u8"filters.lib/name", u8"Faust Filters Library"}, {u8"filters.lib/version", u8"0.0"}, {u8"license", u8"CC0-1.0"}, {u8"maths.lib/author", u8"GRAME"}, {u8"maths.lib/copyright", u8"GRAME"}, {u8"maths.lib/license", u8"LGPL with exception"}, {u8"maths.lib/name", u8"Faust Math Library"}, {u8"maths.lib/version", u8"2.1"}, {u8"name", u8"Stone Phaser"}, {u8"oscillators.lib/name", u8"Faust Oscillator Library"}, {u8"oscillators.lib/version", u8"0.0"}, {u8"signals.lib/name", u8"Faust Signal Routing Library"}, {u8"signals.lib/version", u8"0.0"}, {u8"version", u8"1.2.1"}};
	...
	FMSTATIC const std::size_t active_offsets[] = {(size_t)&((FAUSTCLASS *)0)->fCheckbox0, (size_t)&((FAUSTCLASS *)0)->fHslider3, (size_t)&((FAUSTCLASS *)0)->fHslider2, (size_t)&((FAUSTCLASS *)0)->fHslider0, (size_t)&((FAUSTCLASS *)0)->fHslider1, (size_t)&((FAUSTCLASS *)0)->fHslider4};
